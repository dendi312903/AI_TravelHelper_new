...
if "selected_map_place" not in st.session_state:
    st.session_state.selected_map_place = None
...
def show_place_on_map(place):
    m = folium.Map(location=[place["lat"], place["lon"]], zoom_start=16)
    folium.Marker([place["lat"], place["lon"]], popup=place["name"]).add_to(m)
    st_folium(m, width=700, height=400)

if 'quick_search' in st.session_state:
    city, place_type = st.session_state.quick_search
    with st.spinner(f"Ищем {place_type} в городе {city}..."):
        result = search_places(city, place_type, 1500, 10)
    ...
    if result and 'places' in result:
        ...
        **Для пользователя:** {result.get('user', 'Гость')}
        ...
        if result.get('found', 0) == 0:
            st.warning("😔 К сожалению, ничего не найдено. Попробуйте:")
            st.markdown("""
            - Увеличить радиус поиска
            - Выбрать другой тип места
            ...
            for i, place in enumerate(places, 1):
                ...
                distance_str = f"{int(place.get('distance', 0))} метров"
                if place.get('distance', 0) >= 1000:
                    distance_str = f"{place.get('distance', 0) / 1000:.1f} км"
                ...
                <div style="border-left: 5px solid {border_color}; padding-left: 15px; margin: 20px 0;">
                    ...
                    <p><strong>📏 Расстояние от центра:</strong> {distance_str}</p>
                ...
                col1, col2, col3 = st.columns(3)
                with col1:
                    if st.button(f"🗺️ Показать на карте", key=f"map_btn_{i}"):
                        st.session_state.selected_map_place = place
                        st.rerun()
                
                st.markdown("---")
            
            if st.session_state.selected_map_place is not None:
                st.markdown("---")
                selected = st.session_state.selected_map_place
                st.markdown(f"### 📍 {selected['name']}")
                
                col_info1, col_info2 = st.columns(2)
                with col_info1:
                    st.write(f"**📍 Адрес:** {selected.get('address', 'Не указан')}")
                    st.write(f"**📏 Расстояние:** {selected.get('distance', '?')} м")
                with col_info2:
                    if selected.get('phone'):
                        st.write(f"**📞 Телефон:** {selected['phone']}")
                    if selected.get('website'):
                        st.write(f"**🌐 Сайт:** [{selected['website']}]({selected['website']})")
                
                show_place_on_map(selected)
                
                if st.button("❌ Закрыть карту", use_container_width=True):
                    st.session_state.selected_map_place = None
                    st.rerun()
                st.markdown("---")
